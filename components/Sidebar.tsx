        ...
        <nav className="flex-1 flex flex-col gap-1.5 overflow-y-auto custom-scrollbar pr-2">
          {NAV_ITEMS.map((item) => (
            <div key={item.id} className="flex flex-col gap-1">
              <a
                href={`#${item.id}`}
                className={`group flex items-center justify-between gap-3 px-4 py-3 rounded-2xl transition-all duration-300 border ${
              ...
              <div className={`pl-11 pr-2 flex flex-col gap-1.5 overflow-hidden transition-all duration-500 ${
                activeSection === item.id ? 'max-h-40 opacity-100 py-2' : 'max-h-0 opacity-0'
              }`}>
