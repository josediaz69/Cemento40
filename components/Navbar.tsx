...
const Navbar: React.FC<NavbarProps> = ({ setPage, activePage }) => {
  ...
  const handleNavClick = (page: 'home' | 'proyectos' | 'experiencia' | 'alianzas' | 'contacto') => {
    setPage(page);
          ...
          >
            <button 
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                activePage === 'home' ? 'text-primary bg-primary/5' : 'text-gray-600 hover:text-primary'
              }`}
            >
              Servicios
            ...
            {/* Megamenu Content */}
            <div 
              className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[700px] bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 grid grid-cols-3 gap-8 transition-all duration-300 origin-top ${
                activeDropdown === 'servicios' ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-4 pointer-events-none'
              }`}
            >
                  ...
                  <ul className="flex flex-col gap-2">
                    {section.subItems?.map((sub) => (
                      <li key={sub.id}>
                        <a 
                          href={`#${section.id}`} 
                          onClick={() => { setActiveDropdown(null); handleNavClick('home'); }}
